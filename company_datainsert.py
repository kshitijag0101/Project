...
try:
    ...
    if conn.is_connected():
        query = "SELECT * FROM data_app_bulkfilemodels where is_active = true;"
            ...
            if "indian_" in row.file_name:
                ...
                try:
                    for data in read_file_df.loc[:].itertuples(index=False):
                        ...
                        if data.authorized_capital != "" and not pd.isnull(data.authorized_capital):
                            try:
                                authorized_capital = float(data.authorized_capital)
                            except:
                        ...
                        if data.paid_up_capital != "" and not pd.isnull(data.paid_up_capital):
                            try:
                                paid_up_capital = float(data.paid_up_capital)
                            except:
                ...
                try:
                    for data in read_file_df.loc[:].itertuples(index=False):
                        ...
                        try:
                            number_of_partners = float(data.number_of_partners)
                        except:
                            number_of_partners = None
                        if obligation_of_contribution != "" and not pd.isnull(obligation_of_contribution) and\
                                type(obligation_of_contribution) == float:
                            obligation_of_contribution = float(obligation_of_contribution)
                        else:
                            obligation_of_contribution = None

                        if number_of_partners != "" and not pd.isnull(number_of_partners) and \
